...
from 数字和英文识别模型_resnet import TextRecognition


def save(_model, _best_loss):
    state_dict = _model.state_dict()
    torch.save(state_dict, 'weights/with_peft_model_resnet.pt')
    with open('weights/with_peft_train_meta_resnet.json', 'w') as file:
...
def load():
    ...
    try:
        state_dict = torch.load('weights/with_peft_model_resnet.pt', map_location=device)
        _model.load_state_dict(state_dict)
        with open('weights/with_peft_train_meta_resnet.json', 'r') as file:
...
ds1 = MNIST(root='./data', train=True, download=False, transform=transformers)
ds2 = EMNIST(root='./data', split='letters', train=True, download=False, transform=transformers)
# 合并数据集
...
# 获取需要微调的所有卷积层模块名
conv_module_names = [name for name, module in model.named_modules() if 'conv' in name]
config = peft.LoraConfig(
    r=128,
    target_modules=[name for name, module in model.named_modules() if 'conv' in name] + ['all-linear'],
    modules_to_save=['nr.resnet.fc'],
)
